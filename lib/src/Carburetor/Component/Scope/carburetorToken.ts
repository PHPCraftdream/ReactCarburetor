...
export const carburetorToken = <T extends unknown>(create: () => T, name: string): ICarburetorToken<T> => {
    ...
    if (takenNames.has(name)) {
        throw new Error(
            'Carburetor: a token named "' + name + '" already exists. Two tokens under one name would ' +
            'overwrite each other in a scope and in a dehydrate() payload; give one of them its own name.'
        );
    }
