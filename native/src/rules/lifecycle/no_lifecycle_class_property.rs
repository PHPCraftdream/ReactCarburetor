...
fn fix_for(
    ...
    let type_parameters_text = arrow
        .type_parameters
        .as_deref()
        .map_or("", |declaration| slice(declaration.span));
    let params_text = slice(arrow.params.span);
    let return_type_text = arrow
        .return_type
        .as_deref()
        .map_or("", |annotation| slice(annotation.span));
...
mod tests {
    ...
    fn fixed(source: &str) -> String {
        ...
        let remaining = check(
            &parsed.program,
            &Source {
                path: std::path::Path::new("fixture.tsx"),
                text: &text,
            },
        );
        assert!(
    ...
    fn a_decorated_property_is_reported_without_a_fix() {
        ...
        assert!(
            ...
            "a decorator changes what the rewrite would mean"
        );
    }
