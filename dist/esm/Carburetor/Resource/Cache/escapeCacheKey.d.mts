/**
 * Escapes an already-serialized JSON string into a key safe to use as one path segment.
 *
 ...
 *
 * Split from `encodeCacheKey` (R7-05) so a caller already holding the JSON — `keyOf`'s mutation
 * validation has just computed it — can escape without serializing a second time. Not re-exported
 * from the package entry point: the cache imports it directly.
 */
export declare const escapeCacheKey: (json: string) => string;
